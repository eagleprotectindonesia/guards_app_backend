...
- **`locationName`** → `resolveLocationName(point, attendanceMatchedName, sitePosts)`:
  1. If the site has exactly one `SitePost` → `"Main Site"`.
  2. If the photo has lat/lng → nearest `SitePost.name` (or `"On Site"` if no posts are within range / no posts exist).
  3. Else, if `attendanceMatchedName` is present → use it.
...
| `apps/worker/src/lib/shift-photo-report/aggregate.ts` | Pure helpers (haversine, nearest SitePost, first/last, geofence summary, per-photo geofence + location name) |
...
| `apps/worker/src/lib/shift-photo-report/static-map.ts` | Google Static Maps PNG fetcher (5s timeout; falls back to `null` on error) |
