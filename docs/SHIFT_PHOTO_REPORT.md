...
1. **Cover page**: Eagle Protect logo, "Guard Shift Photo Report" title, Report ID (if available), Report Date (shift start date, WITA), guard info block, shift info block including report status (capitalized, e.g. `Generated`, `Pending`, `Failed`, `Regenerated`).
