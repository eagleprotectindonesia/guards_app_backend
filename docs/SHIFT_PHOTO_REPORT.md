...
- `YYYY-MM-DD` = the date the report was **generated** (WITA timezone, resets at WITA midnight)
...
1. **Cover page**: Eagle Protect logo, "Guard Shift Photo Report" title, Report ID (if available), Report Date (generation date, WITA), guard info block, shift info block including report status (capitalized, e.g. `Generated`, `Pending`, `Failed`, `Regenerated`).
