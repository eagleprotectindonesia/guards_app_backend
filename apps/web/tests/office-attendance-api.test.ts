...
import { OFFICE_ATTENDANCE_CLOCK_OUT_GRACE_HOURS, OFFICE_ATTENDANCE_REQUIRE_PHOTO_SETTING } from '@repo/shared';
...
describe('POST /api/employee/my/office-attendance', () => {
  ...
  test('rejects late clock-out on non-working context when grace anchor is unavailable', async () => {
    ...
    (getLatestOfficeAttendanceForEmployee as jest.Mock).mockResolvedValue({
      id: 'attendance-open',
      employeeId: 'employee-late-clockout',
      status: 'present',
      recordedAt: new Date('2026-05-01T23:00:00.000Z'),
  ...
  test('allows clock-out exactly at grace deadline boundary', async () => {
    jest.useFakeTimers();

    const shiftEnd = new Date('2026-04-02T02:00:00.000Z');
    const graceDeadline = new Date(shiftEnd.getTime() + OFFICE_ATTENDANCE_CLOCK_OUT_GRACE_HOURS * 60 * 60 * 1000);
    jest.setSystemTime(graceDeadline);

    try {
      const openPreviousShiftAttendance = {
        id: 'attendance-open-grace-exact',
        employeeId: 'employee-grace-exact',
        officeId: null,
        officeShiftId: 'office-shift-previous',
      ...
      (getLatestOfficeAttendanceForEmployee as jest.Mock).mockResolvedValue(openPreviousShiftAttendance);
      (recordOfficeAttendance as jest.Mock).mockResolvedValue({
        id: 'attendance-clocked-out-grace-exact',
        employeeId: 'employee-grace-exact',
        officeId: null,
        status: 'clocked_out',
      });
      ...
      expect(response.status).toBe(201);
      expect(data.attendance).toMatchObject({ id: 'attendance-clocked-out-grace-exact', status: 'clocked_out' });
    } finally {
      jest.useRealTimers();
    }
  });
  ...
  test('rejects clock-out when day attendance is present without shift relation and employee open shift is beyond grace', async () => {
    jest.useFakeTimers();

    const now = new Date('2026-04-02T03:00:00.001Z');
    const shiftEnd = new Date('2026-04-02T02:00:00.000Z');
    jest.setSystemTime(now);

    try {
      const dayOpenAttendanceWithoutShiftRelation = {
        id: 'attendance-day-open-no-shift-expired',
        employeeId: 'employee-mismatch-case-expired',
        officeId: null,
        status: 'present',
        recordedAt: new Date('2026-04-02T00:10:00.000Z'),
      };
      const openPreviousShiftAttendance = {
        id: 'attendance-open-previous-shift-expired',
        employeeId: 'employee-mismatch-case-expired',
        officeId: null,
        officeShiftId: 'office-shift-previous-expired',
        businessDate: new Date('2026-04-01T00:00:00.000Z'),
        status: 'present',
        recordedAt: new Date('2026-04-01T18:05:00.000Z'),
        officeShift: {
          id: 'office-shift-previous-expired',
          startsAt: new Date('2026-04-01T18:00:00.000Z'),
          endsAt: shiftEnd,
        },
      };
      ...
      (getAuthenticatedEmployee as jest.Mock).mockResolvedValue({
        id: 'employee-mismatch-case-expired',
        role: 'office',
        officeId: null,
        fieldModeEnabled: true,
      });
  ...
  test('rejects clock-out at one millisecond after grace deadline', async () => {
    ...
    const shiftEnd = new Date('2026-04-02T02:00:00.000Z');
    const graceDeadline = new Date(shiftEnd.getTime() + OFFICE_ATTENDANCE_CLOCK_OUT_GRACE_HOURS * 60 * 60 * 1000);
    jest.setSystemTime(new Date(graceDeadline.getTime() + 1));

    try {
      const openPreviousShiftAttendance = {
        id: 'attendance-open-grace-plus-1ms',
        employeeId: 'employee-grace-plus-1ms',
        officeId: null,
        officeShiftId: 'office-shift-previous',
        businessDate: new Date('2026-04-01T00:00:00.000Z'),
        status: 'present',
  ...
  test('records clock-out against previously open overnight shift when current context is a later shift', async () => {
    ...
    (getLatestOfficeAttendanceInRange as jest.Mock).mockResolvedValue(null);
    (getLatestOfficeAttendanceForDay as jest.Mock).mockResolvedValue(previousShiftOpenAttendance);
    (recordOfficeAttendance as jest.Mock).mockResolvedValue({
      id: 'attendance-overnight-close',
      employeeId: 'employee-shift-handover',
      officeId: null,
      officeShiftId: 'office-shift-overnight',
      status: 'clocked_out',
    ...
    expect(response.status).toBe(201);
    expect(data.attendance).toMatchObject({ id: 'attendance-overnight-close', status: 'clocked_out' });
    expect(recordOfficeAttendance).toHaveBeenCalledWith(
      expect.objectContaining({
        employeeId: 'employee-shift-handover',
        officeShiftId: 'office-shift-overnight',
