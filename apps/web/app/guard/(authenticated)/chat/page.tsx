...
export default function GuardChatPage() {
  ...
  const flushMarkRead = useCallback(() => {
    if (pendingReadIds.current.size === 0 || !socket || !guardId) return;
  ...
  // Socket setup
  useEffect(() => {
    if (!socket || !guardId) return;

    const handleNewMessage = (message: ChatMessage) => {
      console.log('GuardChatPage: New message received via socket', message);
      queryClient.setQueryData<InfiniteData<ChatMessage[]>>(['chat', 'messages', guardId], (old) => {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputText.trim() || !socket || !isConnected || !guardId) return;
