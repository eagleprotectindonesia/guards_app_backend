...
export default function ShiftTypeList({
  ...
  const handleConfirmDelete = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        toast.success('Guard Shift Type deleted successfully!');
        setDeleteId(null);
      } else {
        toast.error(result.message || 'Failed to delete guard shift type.');
      }
    });
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Guard Shift Types</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage standard guard shift templates.</p>
        </div>
        <div className="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
          {canViewAudit && (
            <Link
              href="/admin/guard-shift-types/audit"
              ...
              {shiftTypes.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-muted-foreground">
                    No guard shift types found. Add one to get started.
                  </td>
                </tr>
              ) : (
                shiftTypes.map(shiftType => (
                  <tr key={shiftType.id} className="hover:bg-muted/30 transition-colors group">
                    <td className="py-4 px-6 text-sm text-muted-foreground text-center">{shiftTypes.indexOf(shiftType) + 1 + (page - 1) * perPage}</td>
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-100">
                        <EditButton
                          href={`/admin/guard-shift-types/${shiftType.id}/edit`}
                          disabled={!canEdit}
      ...
      <ConfirmDialog
        ...
        title="Delete Guard Shift Type"
        description="Are you sure you want to delete this guard shift type? This might affect existing guard shifts."
        confirmText="Delete Guard Shift Type"
        isPending={isPending}
      />
    </div>
  );
