...
export function CriticalAlertsCard({ panicAlerts = [] }: CriticalAlertsCardProps) {
  ...
  const mappedPanics = panicAlerts.map(p => ({
    id: `panic-${p.id}`,
    ...
    site: {
      name: `External SOS Alert (${p.latitude.toFixed(4)}, ${p.longitude.toFixed(4)})`,
    },
    isPanic: true,
  }));

  const mergedAlerts = [...mappedPanics, ...criticalAlerts.data];
      ...
      {mergedAlerts.length > 0 && (
        <div className="flex-1 space-y-3 overflow-y-auto pr-1">
          {mergedAlerts.slice(0, 10).map(alert => (
            <div key={alert.id} className={`rounded-lg border border-border p-3 ${alertRowBackgroundClass(alert as any)}`}>
              <div className="mb-1.5 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <AlertTriangle
                    className={`h-3.5 w-3.5 shrink-0 ${alert.severity === 'critical' ? 'text-red-500' : 'text-amber-500'}`}
                  />
                  <span className="truncate text-xs font-semibold uppercase tracking-wide text-foreground/90">{reasonLabel(alert as any)}</span>
                </div>
                <span className={`shrink-0 rounded px-2 py-0.5 text-[10px] font-semibold ${severityBadgeClass(alert as any)}`}>
                  {severityLabel(alert as any)}
                </span>
              </div>

              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 text-sm font-medium text-foreground truncate">{guardSiteLabel(alert as any)}</div>
