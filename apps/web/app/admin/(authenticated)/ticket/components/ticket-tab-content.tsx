      ...
      {activeTab === 'discussion' && (
        <div className="flex-1 flex flex-col min-h-0">
                            ...
                            <div key={att.id} className="w-80 space-y-2">
                              {att.publicUrl && isImageMimeType(att.mimeType) && (
                                <img
                                  src={att.publicUrl}
                                  alt={att.fileName}
                                  className="w-full max-h-52 object-cover rounded-xl border border-border bg-background cursor-pointer hover:opacity-95 transition-opacity"
                                  onClick={() =>
                                    att.publicUrl &&
                                    setActivePreview({
                              ...
                              {att.publicUrl && isVideoMimeType(att.mimeType) && (
                                <video
                                  src={att.publicUrl}
                                  controls
                                  className="w-full max-h-52 rounded-xl border border-border bg-background cursor-pointer"
                                  onClick={() =>
                                    att.publicUrl &&
                                    setActivePreview({
                                      fileName: att.fileName,
                              ...
                              <div
                                className="flex items-center justify-between bg-muted/10 border border-border rounded-xl p-3 hover:border-purple-500/40 transition-colors cursor-pointer"
                                onClick={() =>
                                  att.publicUrl &&
                                  setActivePreview({
                                    fileName: att.fileName,
          ...
          {ticket.status !== 'CLOSED' && ticket.status !== 'CANCELLED' ? (
            ...
            <div className="sticky bottom-0 z-10 p-4 border-t border-border/60 bg-muted/20 text-center text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              This ticket has been closed or cancelled. Discussion is disabled.
            </div>
          )}
        </div>
      )}
          ...
          {ticket.attachments && ticket.attachments.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {ticket.attachments.map(att => (
                <div
                  key={att.id}
                  className="flex items-center justify-between bg-muted/25 border border-border/40 rounded-xl p-3.5 hover:border-purple-500/40 transition-colors cursor-pointer"
                  onClick={() =>
                    att.publicUrl &&
                    setActivePreview({
                      fileName: att.fileName,
      ...
      {activePreview && (
        ...
        >
          <div className="p-6 flex flex-col items-center justify-center bg-background text-foreground min-h-[300px]">
            ...
            {!isImageMimeType(activePreview.mimeType) && !isVideoMimeType(activePreview.mimeType) && (
              <div className="flex flex-col items-center justify-center p-8 border border-dashed border-border rounded-lg bg-muted/10 w-full">
                ...
                <p className="text-sm font-medium text-center break-all">{activePreview.fileName}</p>
                {activePreview.fileSize && (
                  <p className="text-xs text-muted-foreground mt-1">({formatFileSize(activePreview.fileSize)})</p>
                )}
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-4 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm font-medium transition flex items-center gap-2"
                >
