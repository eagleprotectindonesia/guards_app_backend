...
export default function OfficeShiftTypeChangelogFilterModal({
  isOpen,
  onClose,
  onApply,
  initialFilters,
  ...
  const shiftTypeOptions = [
    { value: '', label: 'All Office Shift Types' },
    ...shiftTypes.map(st => ({ value: st.id, label: st.name })),
  ];
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Filter Office Shift Type Audit Logs">
      <div className="flex flex-col justify-between p-4 min-h-[350px]">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">Office Shift Type</label>
            <Select
              options={shiftTypeOptions}
              value={shiftTypeOptions.find(opt => opt.value === entityId)}
              onChange={(val) => setEntityId(val?.value || '')}
              placeholder="All Office Shift Types"
            />
          </div>
