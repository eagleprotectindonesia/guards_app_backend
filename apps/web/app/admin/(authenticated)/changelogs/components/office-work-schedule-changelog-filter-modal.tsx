...
export default function OfficeWorkScheduleChangelogFilterModal({
  isOpen,
  onClose,
  onApply,
  initialFilters,
  ...
  const officeWorkScheduleOptions = [
    { value: '', label: 'All Office Schedules' },
    ...officeWorkSchedules.map(schedule => ({ value: schedule.id, label: schedule.name || 'Unnamed Schedule' })),
  ];
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Filter Office Work Schedule Audit Logs">
      <div className="flex flex-col justify-between p-4 min-h-[350px]">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">Office Schedule</label>
            <Select
              options={officeWorkScheduleOptions}
              value={officeWorkScheduleOptions.find(opt => opt.value === entityId)}
              onChange={val => setEntityId(val?.value || '')}
              placeholder="All Office Schedules"
            />
          </div>
