...
export default function GuardDetail({ guard }: Props) {
  return (
    <div className="bg-card rounded-xl shadow-sm border border-border p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold text-foreground mb-6">Guard Details</h1>
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Guard Code Field */}
          <div>
            <label className="block font-medium text-foreground mb-1">Guard Code</label>
            <div className="w-full h-10 px-3 rounded-lg border border-border bg-muted/50 text-foreground flex items-center">
              {guard.guardCode}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="w-full h-10 px-3 rounded-lg border border-border bg-muted/50 text-foreground flex items-center">
              <span className={`inline-flex items-center ${guard.status ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                <span className={`w-2 h-2 rounded-full mr-2 ${guard.status ? 'bg-green-600 dark:bg-green-400' : 'bg-red-600 dark:bg-red-400'}`}></span>
