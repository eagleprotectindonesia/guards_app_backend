...
import { Check, CheckCheck, MapPin, RefreshCw } from 'lucide-react-native';
    ...
    <View style={[styles.messageContainer, styles.myMessage]}>
      <VStack style={styles.myVStack}>
        <TouchableOpacity onPress={isFailed ? handleRetry : undefined} activeOpacity={isFailed ? 0.6 : 1}>
          <BlurView intensity={40} tint="dark" style={[styles.messageBubble, styles.myBubble, isFailed && styles.failedBubble]}>
            <LinearGradient colors={['rgba(239, 68, 68, 0.15)', 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={StyleSheet.absoluteFill} />
              ...
              <View style={styles.attachmentsContainer}>
                {item.attachmentUris.map((uri, i) => (
                  <View key={i} style={[styles.attachmentWrapper, isFailed && styles.failedAttachment]}>
                    <Image source={{ uri }} style={[styles.attachmentImage, item.attachmentUris.length > 1 && styles.multiAttachmentImage]} resizeMode="cover" />
                  </View>
                ))}
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  myBubble: { backgroundColor: 'rgba(60, 20, 20, 0.4)', borderBottomRightRadius: 4, shadowColor: '#EF4444', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 0 },
  ...
  senderName: { fontSize: 10, color: '#94A3B8', marginBottom: 4, marginLeft: 4, fontWeight: '600', textTransform: 'uppercase' },
  ...
  dateSeparator: { paddingHorizontal: 16, paddingVertical: 6, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.05)', overflow: 'hidden' },
